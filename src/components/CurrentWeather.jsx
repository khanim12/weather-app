...
function CurrentWeather() {
  ...
  const { location, error } = useNavigators();
  useEffect(() => {
      ...
      try {
        const response = await axios(
          `https://api.openweathermap.org/data/2.5/weather?lat=${
            location.latitude
          }&units=metric&lang=az&lon=${location.longitude}&appid=${
    ...
    <div>
      {weather && (
        <div
          ...
          className=" relative bg-[#acdfe7] w-1/3 m-auto  h-80 p-2 mt-12"
        >
          ...
          <div className="text-center w-max capitalize text-[#7d8d8d] m-auto">
            ...
            <h3 className="font-bold font-serif	">
              It's a {weather.weather[0].main} day
            </h3>
            <p className="opacity-55 my-3	">{weather.name}</p>
            <p className="text-2xl my-4">
              {weather.main.temp} C <sup>0</sup>
            </p>
          </div>
        </div>
      )}
